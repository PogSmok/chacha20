...
// Converts std::string to std::vector<std::uint8_t>
std::vector<std::uint8_t> str_to_vec(std::string& str) {
...
// Converts std::vector<std::uint8_t> to std::string
std::string vec_to_str(std::vector<std::uint8_t>& vec) {
    std::string str;
    for(char c : vec) {
        str.push_back(c);
    }
...
void test_case(std::string key, std::string block_count, std::string nonce, std::string message, std::string result) {
    ...
    std::vector<std::uint8_t> coded = cipher.encrypt(str_to_vec(message));
    std::string coded_str = vec_to_str(coded);
    coded_str = str_to_result(coded_str);
    assert(coded_str == result);
}
